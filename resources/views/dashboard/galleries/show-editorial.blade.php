            ...
            <div class="editorial-folder-list">
                @forelse ($gallery->folders as $folder)
                    <label class="editorial-folder-option">
                        ...
                        <span class="editorial-folder-check"><i class="fas fa-check"></i></span>
                        <span class="editorial-folder-icon"><i class="far fa-folder"></i></span>
                        <span class="editorial-folder-name">{{ $folder->name }}</span>
                        <small class="editorial-folder-count">{{ $folder->media->count() }} {{ Str::plural('photo', $folder->media->count()) }}</small>
                    </label>
                @empty
...
<style>
    ...
    .editorial-lightbox, .editorial-drawer-backdrop { position: fixed; inset: 0; opacity: 0; pointer-events: none; transition: opacity .25s ease; } .editorial-lightbox { z-index: 20; display: grid; place-items: center; padding: 0; background: rgba(5,5,4,.96); } .editorial-lightbox.is-open, .editorial-drawer-backdrop.is-visible { opacity: 1; pointer-events: auto; } .editorial-lightbox img { display: block; width: auto !important; height: auto !important; max-width: 95vw; max-height: 95vh; max-height: 95dvh; object-fit: contain; border-radius: 12px; box-shadow: 0 30px 80px rgba(0,0,0,.8); } .editorial-lightbox__close { position: absolute; z-index: 1; top: 1.5rem; right: 1.5rem; display: grid; width: 2.75rem; height: 2.75rem; place-items: center; border: 1px solid #4b4c45; border-radius: 50%; background: transparent; color: #fff; cursor: pointer; }
    ...
    .editorial-folder-option { position: relative; display: grid; grid-template-columns: 20px 32px minmax(0, 1fr) auto; align-items: center; justify-content: initial; gap: .7rem; min-height: 61px; }
    .editorial-folder-option input { position: absolute; opacity: 0; pointer-events: none; }
    .editorial-folder-check { display: grid; width: 19px; height: 19px; place-items: center; border: 1px solid #9da79a; border-radius: 4px; color: transparent; font-size: .58rem; transition: .18s ease; }
    .editorial-folder-option .editorial-folder-check i { margin: 0; color: inherit; }
    .editorial-folder-option input:checked + .editorial-folder-check { border-color: #53604b; background: #53604b; color: #fff; }
    .editorial-folder-icon { display: grid; width: 32px; height: 32px; place-items: center; border-radius: 8px; background: #edf0e8; color: #6f795a; }
    .editorial-folder-option .editorial-folder-icon i { margin: 0; }
    .editorial-folder-name { min-width: 0; overflow: hidden; color: #343a31; text-overflow: ellipsis; white-space: nowrap; }
    .editorial-folder-count { color: #75736d; white-space: nowrap; }

...
<aside class="luxe-drawer" id="luxe-drawer" aria-hidden="true">
    ...
    <form method="POST" action="{{ route('gallery.download', ['photographer_subdomain' => $photographer->subdomain, 'gallery_slug' => $gallery->slug]) }}">@csrf
        <div class="luxe-drawer__body"><p>Choose the folders you would like to receive. We will send your private link by email.</p><label for="luxe-email">Email address <em>*</em></label><input id="luxe-email" name="email" type="email" value="{{ old('email') }}" placeholder="you@example.com" required autocomplete="email">@error('email')<small class="luxe-error">{{ $message }}</small>@enderror
            ...
            <div class="luxe-folders">
                @forelse ($gallery->folders as $folder)
                    <label class="luxe-folder-option">
                        <input class="luxe-folder" name="folder_ids[]" type="checkbox" value="{{ $folder->id }}" {{ in_array($folder->id, old('folder_ids', [])) ? 'checked' : '' }}>
                        <span class="luxe-folder-check"><i class="fas fa-check"></i></span>
                        <span class="luxe-folder-icon"><i class="far fa-folder"></i></span>
                        <span class="luxe-folder-name">{{ $folder->name }}</span>
                        <small class="luxe-folder-count">{{ $folder->media->count() }} {{ Str::plural('photo', $folder->media->count()) }}</small>
                    </label>
                @empty
                    <p>No folders are available for download yet.</p>
                @endforelse
            </div>
            @error('folder_ids')<small class="luxe-error">{{ $message }}</small>@enderror
...
.luxe-folder-option{position:relative!important;display:grid!important;grid-template-columns:20px 32px minmax(0,1fr) auto;align-items:center;justify-content:initial!important;gap:.7rem!important;min-height:61px}
.luxe-folder-option input{position:absolute;opacity:0;pointer-events:none}
.luxe-folder-check{display:grid;width:19px;height:19px;place-items:center;border:1px solid #736654;color:transparent;font-size:.58rem;transition:.18s ease}
.luxe-folder-option .luxe-folder-check i{margin:0;color:inherit}
.luxe-folder-option input:checked + .luxe-folder-check{border-color:#c9a775;background:#c9a775;color:#211a13}
.luxe-folder-icon{display:grid;width:32px;height:32px;place-items:center;background:rgba(201,167,117,.11);color:#c9a775}
.luxe-folder-option .luxe-folder-icon i{margin:0}
.luxe-folder-name{min-width:0;overflow:hidden;color:#f0e6d8;text-overflow:ellipsis;white-space:nowrap}
.luxe-folder-count{color:#a99d90;white-space:nowrap}
</style>
